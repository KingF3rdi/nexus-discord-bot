...
function productLabel(ticket: PurchaseTicket): string {
  if (ticket.product_name) return ticket.product_name;
  if (ticket.product_id) {
    const row = db.prepare("SELECT name FROM products WHERE id = ?").get(ticket.product_id) as
      | { name: string }
      | undefined;
    if (row?.name) return row.name;
  }
  if (ticket.type.startsWith("spawner")) return "Spawner";
  return "Kauf";
}

export function shouldAskVouch(ticket: { type: string; total: number | null }) {
  if (!ticket.total || ticket.total <= 0) return false;
  if (ticket.type === "clan" || ticket.type === "spawner-sell") return false;
  return true;
}

export async function sendVouchDm(client: Client, ticket: PurchaseTicket): Promise<"sent" | "skipped" | "failed"> {
  if (!shouldAskVouch(ticket)) return "skipped";
  const existing = db
    .prepare("SELECT id FROM vouch_requests WHERE ticket_id = ?")
    .get(ticket.id) as { id: number } | undefined;
  if (existing) return "skipped";
  const result = db
    .prepare(
      `INSERT INTO vouch_requests (guild_id, ticket_id, user_id, product, quantity, price, seller_id, status, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?)`,
    )
    .run(
      ticket.guild_id,
...
export async function handleVouchDmButton(interaction: ButtonInteraction) {
  ...
  const req = db.prepare("SELECT * FROM vouch_requests WHERE id = ?").get(requestId) as
    | {
        id: number;
        guild_id: string;
        user_id: string;
    ...
    | undefined;
  if (!req) throw new Error("Diese Bewertungsanfrage gibt es nicht mehr.");
  if (req.user_id !== interaction.user.id) throw new Error("Das ist nicht deine Bewertung.");
  ...
  const total = (db.prepare("SELECT COUNT(*) AS c FROM vouches WHERE guild_id = ?").get(req.guild_id) as { c: number }).c;
  const categoryCount = (
    db.prepare("SELECT COUNT(*) AS c FROM vouches WHERE guild_id = ? AND category = ?").get(req.guild_id, "Shop") as {
      c: number;
    }
  ).c;
  const payload = vouchEmbed(
    config,
    {
      id: vouchId,
      category: "Shop",
