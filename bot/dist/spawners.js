import { ActionRowBuilder, EmbedBuilder, ModalBuilder, parseEmoji, StringSelectMenuBuilder, TextInputBuilder, TextInputStyle, } from "discord.js";
import { db, ensureDefaultSpawners, getGuild, listSpawners, requireGuildId, updateGuild, } from "./db.js";
...
function panelPayload(guildId, notifyChannelId) {
...
export async function refreshSpawnerPanels(client, guildId) {
    const rows = db
...
function upsertSpawner(guildId, name, buy, sell, emoji, mode = "upsert") {
    ...
    if (existing) {
        db.prepare("UPDATE spawners SET buy_price = ?, sell_price = ?, emoji = ? WHERE id = ?").run(buy, sell, mark, existing.id);
        return "updated";
    }
    db.prepare("INSERT INTO spawners (guild_id, name, buy_price, sell_price, sort_order, emoji) VALUES (?, ?, ?, ?, 99, ?)").run(guildId, name, buy, sell, mark);
    return "created";
}
export async function cmdSpawner(interaction) {
...
export async function cmdSpawnerPanel(interaction) {
    const guildId = requireGuildId(interaction.guildId);
    ensureDefaultSpawners(guildId);
    ...
    db.prepare("INSERT INTO panels (guild_id, type, channel_id, message_id, extra) VALUES (?, 'spawner', ?, ?, ?)").run(guildId, channel.id, msg.id, notify?.id ?? null);
    await interaction.reply({ content: `Spawner-Panel in ${channel} gesendet.`, flags: 64 });
}
export async function openSpawnerPicker(interaction, direction) {
    const guildId = requireGuildId(interaction.guildId);
    ensureDefaultSpawners(guildId);
    ...
    if (!spawners.length) {
        await interaction.reply({
            embeds: [
                warningEmbed(direction === "buy"
                    ? "Aktuell sind keine Spawner im Verkauf (alles auf STOP)."
                    : "Aktuell kaufen wir keine Spawner an.", COLORS.yellow),
            ],
            flags: 64,
        });
        return;
    ...
    await interaction.reply({
        ...
        components: [
            new ActionRowBuilder().addComponents(new StringSelectMenuBuilder()
                ...
                .addOptions(spawners.slice(0, 25).map((s) => {
...
export async function openSpawnerQtyModal(interaction) {
    const direction = interaction.customId.endsWith(":buy") ? "buy" : "sell";
    const id = Number(interaction.values[0]);
    const spawner = db.prepare("SELECT * FROM spawners WHERE id = ?").get(id);
    if (!spawner)
        throw new Error("Spawner nicht gefunden.");
    const price = direction === "buy" ? spawner.sell_price : spawner.buy_price;
    if (price == null)
        throw new Error(`**${spawner.name}** ist für diese Richtung auf STOP.`);
...
export async function openSpawnerTicket(interaction, direction, spawnerId) {
    const guildId = requireGuildId(interaction.guildId);
    const spawner = db.prepare("SELECT * FROM spawners WHERE id = ? AND guild_id = ?").get(spawnerId, guildId);
    if (!spawner)
        throw new Error("Spawner nicht gefunden.");
    const unit = direction === "buy" ? spawner.sell_price : spawner.buy_price;
    if (unit == null)
        throw new Error(`**${spawner.name}** ist auf STOP.`);
    const qty = Number(interaction.fields.getTextInputValue("qty"));
    if (!Number.isInteger(qty) || qty < 1 || qty > 999)
        throw new Error("Bitte eine Menge zwischen 1 und 999 eingeben.");
    const ign = interaction.fields.getTextInputValue("ign").trim();
    if (!ign)
        throw new Error("Minecraft-Name fehlt.");
